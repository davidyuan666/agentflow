...
class AgentNameConflict(Exception):
    def __init__(self, msg: str = "Found multiple auto_agent with the same name.", *args: Any, **kwargs: Any):
...
class UndefinedNextAgent(Exception):
    """Exception raised when the provided next auto_agent list does not overlap with auto_agent in the group."""

    def __init__(self, message: str = "The provided auto_agent list does not overlap with auto_agent in the group."):
        self.message = message
        super().__init__(self.message)
